...
created: 2026-08-27
updated: 2026-08-27T22:28:06+07:00
---
...
- hypothesis: Confirmed resolved: the deterministic QEMU TCG IO-APIC timer failure is bypassed by verified direct-kernel boot with the pinned image command line plus one `noapic`.
- test: Record the accepted machine-verifiable confirmation, archive this debug session, and append its redacted resolution and prevention guard to the durable knowledge base.
- expecting: Debug records are committed without launching package transfer, product build, signing, ISO smoke, or additional QEMU activity.
- next_action: Move this file to `.planning/debug/resolved/`, append `.planning/debug/knowledge-base.md`, and commit those documentation changes.
- bug_class: bohrbug
...
- timestamp: 2026-08-27T22:28:06+07:00
  checked: Human verification checkpoint response
  found: The parent explicitly confirmed the fix and accepted the machine-verifiable readiness markers, strict SSH, fingerprint verification, cleanup proof, test suite, and immutable publication hashes as sufficient verification under the unattended deadline instruction.
  implication: The original pre-readiness blocker is resolved and the session may be archived; no product build or ISO smoke is required or authorized in this debug session.
...
- verification:
    ...
    human_confirmation: { result: pass, response: confirmed fixed, basis: machine-verifiable readiness and cleanup evidence accepted }
...
- branched_5_whys:
    - environment: QEMU 11.1.0 TCG on this Windows host could not deliver Alpine 6.18.35 IRQ0 through the IO-APIC; because no usable alternate hypervisor or Linux build runtime was available, the owned builder path had to remain on TCG.
    - config: The pinned image booted only through Extlinux with an APPEND line that lacked `noapic`; because the backend did not expose a verified direct-kernel command-line seam, it could not apply the kernel-prescribed workaround without mutating the immutable base.
    - code_and_gate: Both conditions were required—the host-specific TCG timer failure and a boot configuration that always entered the IO-APIC path—so changing q35 to pc alone could not resolve the panic.
- why_not_caught: No pre-existing static or readiness gate covered preservation of the pinned Extlinux command line while adding a host-specific kernel workaround; the fault appeared before project markers, so later build, signing, inspection, and smoke gates could not observe it.
- recurrence_guard: The regression test `tests/deadline/run.ps1` named `Builder direct boot preserves the pinned image contract and adds noapic once` verifies exact APPEND preservation and exactly one `noapic`; the runtime extraction path also fails closed on missing or duplicate APPEND lines before QEMU launch.
