...
Add-Test -Name 'Locked live-user boot path is bounded and retains rescue access' -Scopes @('RuntimeStatic') -Body {
    ...
    Assert-Match $runtime '(?ms)300k-autologin\)\s+\[ "\$#" -eq 1 \] \|\| usage\s+\[ "\$1" = -- \] \|\| usage\s+shift\s+action=autologin' 'Symlink basename dispatch must consume exactly BusyBox getty''s one literal -- sentinel; zero, additional, or non-sentinel arguments must fail closed.'
