...
Add-TestCase -Name 'BUILD-04 repository drift aborts before the offline install boundary' -Scopes @('Unit') -Requirements @('BUILD-04') -Body {
    ...
    Assert-Match -Value $linux -Pattern 'assert_exact_xorriso_stderr[\s\S]*banner_count[\s\S]*-le 1' -Message 'Every xorriso caller must share one exact locked-banner stderr contract.'
