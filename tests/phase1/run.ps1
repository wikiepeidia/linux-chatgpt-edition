...
Add-TestCase -Name 'BUILD-03 hostile fixture matrix preflights before no-follow streaming' -Scopes @('Unit') -Requirements @('BUILD-03') -Body {
    ...
    Assert-Match -Value $source -Pattern ([regex]::Escape($modloopSignatureLogicalPath + ')') + '[\s\S]{0,300}assert_project_modloop_signature_shape[\s\S]{0,300}\*[.]rsa[.]pub\)') -Message 'The exact modloop signature arm must precede and preserve the generic public-key hash allowlist.'
