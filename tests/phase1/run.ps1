...
Add-TestCase -Name 'BUILD-03 hostile fixture matrix preflights before no-follow streaming' -Scopes @('Unit') -Requirements @('BUILD-03') -Body {
    ...
    Assert-Match -Value $source -Pattern '\$iso_command" -report_about WARNING -indev "\$container" -concat overwrite - "/\$member" --' -Message 'ISO members must stream to the bounded stdout sink without enabling xorriso disk restoration.'
    Assert-False -Condition ($source -match '-osirrox|-extract_single') -Message 'The inspector must not enable xorriso disk restoration when a bounded stdout decoder exists.'
...
Add-TestCase -Name 'BUILD-04 repository drift aborts before the offline install boundary' -Scopes @('Unit') -Requirements @('BUILD-04') -Body {
    ...
    Assert-Match -Value $linux -Pattern 'append_iso_report[\s\S]*expected_banner=\$\(inspection_evidence_value "\$evidence" iso version\)[\s\S]*banner_count[\s\S]*-le 1[\s\S]*BOOT_LAYOUT_REPORT_FAILED' -Message 'Boot reports must validate status and strict stderr against locked xorriso evidence.'
