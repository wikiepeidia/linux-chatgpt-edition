...
Add-TestCase -Name 'BUILD-03 hostile fixture matrix preflights before no-follow streaming' -Scopes @('Unit') -Requirements @('BUILD-03') -Body {
    ...
    Assert-Match -Value $source -Pattern 'inspect_archive_members[\s\S]*while IFS="\$tab" read -r id type size member target; do[\s\S]*\([\s\S]*materialized=\$\(stream_regular_member[\s\S]*member_logical=\$logical/\$member[\s\S]*inspect_file "\$materialized" "\$member_logical"[\s\S]*\)[\s\S]*done < "\$manifest"' -Message 'Each recursive member inspection must run in a subshell so child traversal variables cannot clobber the parent container loop.'
    Assert-Match -Value $source -Pattern 'decoder_error_bytes=.*file_bytes[\s\S]*decoder_error_sha256=.*sha256_file[\s\S]*head -c 256[\s\S]*INSPECTION_DECODE_FAILED.*exit=\$status.*stderr_bytes=\$decoder_error_bytes.*stderr_sha256=\$decoder_error_sha256' -Message 'A failed bounded member decoder must retain a sanitized, size- and hash-bound diagnostic before cleanup.'
    Assert-Match -Value $source -Pattern 'INSPECTION_SECRET_FOUND.*decoded regular bytes contain a private credential or host-management marker logical=\$logical' -Message 'Secret-scan failures must identify only the preflight-approved logical member path, never matched credential bytes.'
    Assert-Match -Value $source -Pattern 'contains_private_key\(\)[\s\S]{0,1800}\^-----BEGIN \[A-Z0-9 \]\*PRIVATE KEY-----\$[\s\S]{0,900}length\(line\) >= 16[\s\S]{0,900}-----END " label "-----' -Message 'Private-key detection must require a line-framed header, plausible payload, and matching footer instead of a binary template substring.'
    Assert-False -Condition ([bool]($source -match "private_pattern='BEGIN \[A-Z0-9 \]\*PRIVATE KEY'")) -Message 'A loose private-key marker substring must not reject signed binaries that implement key serialization.'
    ...
    $modloopSignatureLogicalPath = 'image.iso/boot/initramfs-virt.decoded/var/cache/misc/modloop-virt.SIGN.RSA.300k.rsa.pub'
    Assert-Match -Value $source -Pattern ([regex]::Escape($modloopSignatureLogicalPath)) -Message 'The exact Alpine initramfs modloop-signature location is not classified separately from public APK keys.'
    Assert-Match -Value $source -Pattern 'assert_project_modloop_signature_shape\(\)[\s\S]{0,1800}file_bytes "\$file"[\s\S]{0,500}-eq 512' -Message 'The exact modloop exception does not require the RSA-4096 detached-signature width.'
    Assert-Match -Value $source -Pattern 'assert_project_modloop_signature_shape\(\)[\s\S]{0,1800}awk[\s\S]{0,500}300k[.]rsa[.]pub[\s\S]{0,500}-eq 1' -Message 'The modloop signature suffix is not bound to exactly one already-approved project signer record.'
    Assert-Match -Value $source -Pattern 'assert_project_modloop_signature_shape\(\)[\s\S]{0,1800}BEGIN \(RSA \)\?PUBLIC KEY[\s\S]{0,500}INSPECTION_MODLOOP_SIGNATURE_INVALID' -Message 'A PEM public key could be misclassified as the opaque modloop signature.'
    Assert-Match -Value $source -Pattern ('assert_project_modloop_signature_shape\(\)[\s\S]{0,1800}' + [regex]::Escape('[[:space:]]*$')) -Message 'The PEM-frame guard does not consume CRLF or trailing line whitespace fail-closed.'
    Assert-Match -Value $source -Pattern 'PROJECT_MODLOOP_SIGNATURE_LOGICAL=[^\r\n]+[\s\S]{0,160}is_project_modloop_signature_path\(\)[\s\S]{0,160}\[ "\$1" = "\$PROJECT_MODLOOP_SIGNATURE_LOGICAL" \]' -Message 'One side-effect-free exact-path predicate must identify the project modloop signature.'
    Assert-Match -Value $source -Pattern 'scan_regular_bytes\(\)[\s\S]{0,1800}is_project_modloop_signature_path "\$logical"[\s\S]{0,300}assert_project_modloop_signature_shape "\$file"[\s\S]{0,500}\*[.]rsa[.]pub\)' -Message 'Byte scanning must share the exact modloop-signature predicate while preserving the generic public-key hash allowlist.'
    Assert-Match -Value $source -Pattern 'role_for_path\(\)[\s\S]{0,240}logical=\$2[\s\S]{0,240}is_project_modloop_signature_path "\$logical"[\s\S]{0,240}printf ''%s\\n'' auto[\s\S]{0,500}\*/modloop-\*\|modloop-\*\)' -Message 'Role inference must apply the same exact-path predicate before the broad modloop container rule.'
    Assert-Match -Value $source -Pattern 'member_logical=\$logical/\$member[\s\S]{0,240}role=\$\(role_for_path "\$member" "\$member_logical"\)' -Message 'Recursive traversal must provide both the member path and full logical path to role inference.'
    Assert-Match -Value $source -Pattern 'valid-modloop-signature[\s\S]{0,600}scan_regular_bytes' -Message 'The runtime self-test does not accept the exact opaque 512-byte modloop signature representation.'
    Assert-Match -Value $source -Pattern 'valid-modloop-signature[\s\S]{0,1400}role_for_path "\$modloop_signature_member" "\$modloop_signature_logical"[\s\S]{0,500}inspect_file "\$valid_modloop_signature" "\$modloop_signature_logical" 0 "\$valid_modloop_signature_role"' -Message 'The exact 512-byte opaque signature must pass byte scanning and inferred-role leaf inspection end to end.'
    Assert-Match -Value $source -Pattern 'near-miss-modloop-signature-role[\s\S]{0,500}expect_role modloop' -Message 'A near-miss signature location must retain the fail-closed modloop container role.'
    Assert-Match -Value $source -Pattern 'genuine-modloop-container-role[\s\S]{0,500}expect_role modloop' -Message 'A genuine boot modloop path must retain the modloop container role.'
    Assert-False -Condition ([bool]($source -match 'dd if=/dev/zero of="\$valid_modloop_signature"')) -Message 'The nested builder self-test runs before /dev is mounted and must not depend on /dev/zero.'
    Assert-Match -Value $source -Pattern 'valid_modloop_signature=\$root/content/valid-modloop-signature[\s\S]{0,240}awk ''BEGIN \{ for \(i = 0; i < 512; i\+\+\) printf "S" \}'' > "\$valid_modloop_signature"[\s\S]{0,240}file_bytes "\$valid_modloop_signature"[\s\S]{0,120}-eq 512' -Message 'The valid opaque signature fixture must be generated as exactly 512 portable non-PEM bytes.'
    Assert-Match -Value $source -Pattern 'awk ''BEGIN \{ for \(i = 0; i < 512; i\+\+\) printf "S" \}'' > "\$valid_modloop_signature"[\s\S]{0,160}fail SELF_TEST_FIXTURE_INVALID "valid modloop signature fixture generation failed"' -Message 'Portable fixture generation must fail with one sanitized self-test code.'
    Assert-Match -Value $source -Pattern 'short-modloop-signature[\s\S]{0,700}INSPECTION_MODLOOP_SIGNATURE_INVALID' -Message 'The runtime self-test does not reject a short exact-path modloop signature.'
    Assert-Match -Value $source -Pattern 'pem-modloop-signature[\s\S]{0,700}INSPECTION_MODLOOP_SIGNATURE_INVALID' -Message 'The runtime self-test does not reject public-key PEM bytes at the modloop signature path.'
    Assert-Match -Value $source -Pattern 'crlf-pem-modloop-signature[\s\S]{0,700}INSPECTION_MODLOOP_SIGNATURE_INVALID' -Message 'The runtime self-test does not reject a CRLF public-key frame at the modloop signature path.'
    Assert-Match -Value $source -Pattern 'wrong-path-modloop-signature[\s\S]{0,700}INSPECTION_PUBLIC_KEY_UNAPPROVED' -Message 'The runtime self-test does not prove that ordinary unapproved public-key paths remain rejected.'
    ...
    Assert-Match -Value $source -Pattern 'if \(type == "d" && \(path == "[.]" \|\| path == "[.]/"\)\)[\s\S]{0,240}cpio_root_seen\+\+[\s\S]{0,240}duplicate CPIO archive root[\s\S]{0,120}next' -Message 'Only the CPIO parser may discard exactly one virtual archive-root directory sentinel before global path validation.'
    ...
    Assert-Match -Value $source -Pattern ([regex]::Escape("printf '%s\n' . . payload")) -Message 'The runtime self-test must reject a duplicate CPIO virtual-root directory sentinel.'
    ...
    Assert-Match -Value $source -Pattern 'target ~ /\^\\/\+/[\s\S]{0,180}sub\(/\^\\/\+/, "", target\)[\s\S]{0,180}base="[.]"' -Message 'Absolute package symlinks must canonicalize inside the virtual archive root, never the host filesystem.'
    Assert-Match -Value $source -Pattern 'paths\[path\] == "h"[\s\S]{0,240}!\(target in paths\)[\s\S]{0,240}hardlink target' -Message 'Only hardlinks must retain same-archive target presence and regular-file type enforcement.'
    Assert-Match -Value $source -Pattern 'if \(type == "h"\) canonical=resolve\("[.]",target\)[\s\S]{0,160}else canonical=resolve\(path,target\)' -Message 'Tar hardlink linknames must resolve from the archive root while symlink targets remain parent-relative.'
    Assert-False -Condition ([bool]($source -match 'if \(!\(target in paths\)\) die\("link target is not present')) -Message 'Never-materialized symlinks may name a root-confined target supplied by another signed APK or runtime filesystem.'
    Assert-Match -Value $source -Pattern 'root-confined-external-symlinks[\s\S]{0,600}var/lock[\s\S]{0,600}[.][.]/run/lock[\s\S]{0,600}bin/sh[\s\S]{0,600}/bin/busybox[\s\S]{0,600}preflight_graph' -Message 'The self-test must positively cover relative and absolute virtual-root symlinks with external targets.'
    Assert-Match -Value $source -Pattern 'root-relative-hardlink[\s\S]{0,800}usr/sbin/ntpd[\s\S]{0,300}usr/sbin/ntpctl[\s\S]{0,300}preflight_graph' -Message 'The self-test must positively cover a tar hardlink whose linkname is archive-root-relative.'
    Assert-Match -Value $source -Pattern 'windows_unc_pattern=.*\[A-Za-z0-9[.]_-\]\+.*\[A-Za-z0-9\\\$[.]_-\]\+' -Message 'UNC detection must require bounded Windows host/share component alphabets instead of arbitrary backslash escapes.'
    Assert-Match -Value $source -Pattern 'clean-escaped-shell[\s\S]{0,500}inspect_file' -Message 'The runtime self-test must accept shell escape syntax that is not a Windows UNC host path.'
    Assert-Match -Value $source -Pattern 'host-unc[\s\S]{0,500}expect_probe_failure INSPECTION_SECRET_FOUND' -Message 'The runtime self-test must still reject a constructed Windows UNC host path.'
    Assert-Match -Value $source -Pattern 'stderr_banner=\$\(evidence_field iso stderr_banner\)[\s\S]*stderr_banner_framing=\$\(evidence_field iso stderr_banner_framing\)[\s\S]*stderr_banner_sha256=\$\(evidence_field iso stderr_banner_sha256\)[\s\S]*printf ''%s\\n\\n''[\s\S]*sha256_file "\$expected"[\s\S]*cmp "\$log" "\$expected"' -Message 'Only the separately locked, reconstructed, hash-checked xorriso banner frame may be removed from strict decoder stderr.'
...
Add-TestCase -Name 'BUILD-03 offline build proves decoder identity before inspecting and publishing' -Scopes @('Unit') -Requirements @('BUILD-03') -Body {
    ...
    Assert-Match -Value $linuxCore -Pattern 'inspector_self_test_diagnostic\(\)[\s\S]{0,1800}\^\[A-Z\]\[A-Z0-9_\]\*:[\s\S]{0,500}count == 1' -Message 'The build must accept exactly one uppercase diagnostic code, never an arbitrary self-test log line.'
    Assert-Match -Value $linuxCore -Pattern 'candidate !~ /\^INSPECTION_\[A-Z0-9_\][+][\$]/ && candidate !~ /\^SELF_TEST_\[A-Z0-9_\][+][\$]/' -Message 'Only the two closed inspector diagnostic namespaces may cross the host failure channel.'
    Assert-Match -Value $linuxCore -Pattern 'length\(candidate\) > 64[\s\S]{0,900}grep -F "fail \$code " "\$inspector"' -Message 'A self-test diagnostic must be length-bounded and already present in the trusted inspector fail-code allowlist.'
...
function New-ClosedSecurityFixture {
    ...
    $fixtureVersionBytes = [System.Text.UTF8Encoding]::new($false).GetBytes("xorriso 1.5.8`nlibburnia fixture detail`n")
    $fixtureVersionHex = [Convert]::ToHexString($fixtureVersionBytes).ToLowerInvariant()
    $fixtureVersionHash = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($fixtureVersionBytes)).ToLowerInvariant()
    $toolItems = @($inputs.inspection_toolchain.PSObject.Properties | ForEach-Object {
        $record = [ordered]@{
            ...
            command_sha256 = ('f' * 64); version = $(if ($_.Name -ceq 'iso') { $_.Value.version_identity } else { 'fixture' }); package_ownership_verified = $true
        ...
        if ($_.Name -ceq 'iso') {
            $record['version_stdout_hex'] = $fixtureVersionHex
            $record['version_stdout_sha256'] = $fixtureVersionHash
            $record['version_stdout_bytes'] = [long]$fixtureVersionBytes.Length
            $record['stderr_banner'] = $_.Value.stderr_banner
            $record['stderr_banner_framing'] = $_.Value.stderr_banner_framing
            $record['stderr_banner_bytes'] = [long]$_.Value.stderr_banner_bytes
            $record['stderr_banner_sha256'] = $_.Value.stderr_banner_sha256
        }
        $record
    })
    $toolchainHash = '9' * 64
...
Add-TestCase -Name 'BUILD-03 closed staging rejects paths schemas hashes sizes links secrets and extras' -Scopes @('Unit') -Requirements @('BUILD-03') -Body {
    ...
    try {
        ...
        $lockPath = Join-Path $fixture.Staging 'resolved-build-lock.json'
        $lockBytes = [System.IO.File]::ReadAllBytes($lockPath)
        $lock = Get-Content -Raw -LiteralPath $lockPath | ConvertFrom-Json -Depth 64
        $isoCommand = @($lock.inspection_commands | Where-Object format -CEQ 'iso')[0]
        $wrongVersionBytes = [System.Text.UTF8Encoding]::new($false).GetBytes("xorriso 9.9.9`nlibburnia fixture detail`n")
        $isoCommand.version_stdout_hex = [Convert]::ToHexString($wrongVersionBytes).ToLowerInvariant()
        $isoCommand.version_stdout_sha256 = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($wrongVersionBytes)).ToLowerInvariant()
        $isoCommand.version_stdout_bytes = [long]$wrongVersionBytes.Length
        Write-CanonicalJson -Value $lock -Path $lockPath
        Assert-ThrowsCode -Code 'INSPECTION_TOOLCHAIN_INVALID' -Body { Test-ClosedStagingBundle -StagingDirectory $fixture.Staging -ExpectedBuildRequestSha256 $fixture.RequestHash -Inputs $fixture.Inputs -BuildRequest $fixture.Request | Out-Null }
...
Add-TestCase -Name 'BUILD-04 public inputs pin the complete builder and inspection command surface' -Scopes @('Unit') -Requirements @('BUILD-04') -Body {
    ...
    Assert-Equal -Expected '34a683f88ed4c897943b5bacf9c388d8a6b5980296ddddaa35fac1417fdf4deb' -Actual $repositoryPins['community'] -Message 'The twice-observed official v3.24 community APKINDEX bytes are not pinned.'
    ...
    Assert-Equal -Expected 'xorriso 1.5.8' -Actual $inputs.inspection_toolchain.iso.version_identity -Message 'The canonical xorriso identity must remain distinct from full version stdout and stderr.'
    Assert-Equal -Expected 'xorriso 1.5.8 : RockRidge filesystem manipulator, libburnia project.' -Actual $inputs.inspection_toolchain.iso.stderr_banner -Message 'The accepted xorriso stderr banner line must be exact and public.'
    Assert-Equal -Expected ($inputs.inspection_toolchain.iso.version_identity + ' : RockRidge filesystem manipulator, libburnia project.') -Actual $inputs.inspection_toolchain.iso.stderr_banner -Message 'The banner line is not deterministically bound to the canonical first line of xorriso -version stdout.'
    Assert-Equal -Expected 'lf-lf' -Actual $inputs.inspection_toolchain.iso.stderr_banner_framing -Message 'The exact xorriso stderr banner framing must be pinned.'
    $framedBanner = [System.Text.UTF8Encoding]::new($false).GetBytes(([string]$inputs.inspection_toolchain.iso.stderr_banner + "`n`n"))
    Assert-Equal -Expected 70 -Actual ([long]$inputs.inspection_toolchain.iso.stderr_banner_bytes) -Message 'The pinned banner byte count differs from the observed xorriso stderr frame.'
    Assert-Equal -Expected ([long]$inputs.inspection_toolchain.iso.stderr_banner_bytes) -Actual ([long]$framedBanner.Length) -Message 'The serialized banner line/framing does not reconstruct the pinned byte count.'
    Assert-Equal -Expected 'f2f1edcb0b5c04de61066b76942ae38fc2fdb1ebd1978acfe5108bad01e9c64f' -Actual $inputs.inspection_toolchain.iso.stderr_banner_sha256 -Message 'The exact xorriso stderr frame hash must be pinned.'
    Assert-Equal -Expected $inputs.inspection_toolchain.iso.stderr_banner_sha256 -Actual ([Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($framedBanner)).ToLowerInvariant()) -Message 'The pinned banner line/framing differs from the observed xorriso stderr bytes.'
...
Add-TestCase -Name 'BUILD-04 repository drift aborts before the offline install boundary' -Scopes @('Unit') -Requirements @('BUILD-04') -Body {
    ...
    Assert-Match -Value $linux -Pattern 'assert_exact_xorriso_stderr[\s\S]*stderr_banner_framing[\s\S]*stderr_banner_bytes[\s\S]*stderr_banner_sha256[\s\S]*cmp "\$log" "\$expected"[\s\S]*residue_bytes[\s\S]*residue_hash[\s\S]*head -c 256' -Message 'Every xorriso caller must reconstruct and hash-check the exact framed locked banner before accepting only empty or byte-equal stderr.'
    Assert-Match -Value $linux -Pattern 'version_stdout=.*inspection-version-iso[.]stdout[\s\S]*version_stderr=.*inspection-version-iso[.]stderr[\s\S]*-version > "\$version_stdout" 2> "\$version_stderr"' -Message 'xorriso -version stdout and stderr must be captured as separate byte streams.'
    Assert-Match -Value $linux -Pattern 'version_stdout_hex=.*od -An -tx1 -v[\s\S]*version_stdout_sha256=.*sha256_file[\s\S]*version_stdout_bytes=.*file_bytes' -Message 'The complete xorriso -version stdout must be serialized and hash-bound without linewise collapse.'
    Assert-Match -Value $linux -Pattern 'version_identity=\$\(sed -n ''1p'' "\$version_stdout"\)[\s\S]*expected_identity=\$\(inspection_value iso version_identity\)[\s\S]*stderr_banner="\$version_identity : RockRidge filesystem manipulator, libburnia project[.]"' -Message 'The pinned stderr banner line must derive deterministically from the canonical first stdout line.'
    ...
    Assert-Match -Value $linux -Pattern 'append_iso_report[\s\S]*inspection_evidence_value "\$evidence" iso stderr_banner[\s\S]*inspection_evidence_value "\$evidence" iso stderr_banner_sha256[\s\S]*assert_exact_xorriso_stderr[\s\S]*BOOT_LAYOUT_REPORT_FAILED' -Message 'Boot reports must validate status and strict stderr against separately locked xorriso banner evidence.'
...
if ($Scope -in @('Qemu', 'Security', 'All')) {
    Add-TestCase -Name 'BUILD-03 BUILD-04 real clean-tree decoded-security QEMU tracer' -Scopes @('Qemu', 'Security') -Requirements @('BUILD-03', 'BUILD-04') -Body {
        ...
        $isoCommand = @($commands | Where-Object format -CEQ 'iso')[0]
        Assert-Equal -Expected $inputs.inspection_toolchain.iso.version_identity -Actual $isoCommand.version -Message 'Resolved xorriso identity differs from the canonical first version line.'
        Assert-Equal -Expected $inputs.inspection_toolchain.iso.stderr_banner -Actual $isoCommand.stderr_banner -Message 'Resolved xorriso stderr banner line differs from the public pin.'
        Assert-Equal -Expected $inputs.inspection_toolchain.iso.stderr_banner_framing -Actual $isoCommand.stderr_banner_framing -Message 'Resolved xorriso stderr framing differs from the public pin.'
        Assert-Equal -Expected ([long]$inputs.inspection_toolchain.iso.stderr_banner_bytes) -Actual ([long]$isoCommand.stderr_banner_bytes) -Message 'Resolved xorriso stderr byte count differs from the public pin.'
        Assert-Equal -Expected $inputs.inspection_toolchain.iso.stderr_banner_sha256 -Actual $isoCommand.stderr_banner_sha256 -Message 'Resolved xorriso stderr hash differs from the public pin.'
        $versionBytes = [Convert]::FromHexString([string]$isoCommand.version_stdout_hex)
        Assert-Equal -Expected ([long]$isoCommand.version_stdout_bytes) -Actual ([long]$versionBytes.Length) -Message 'Serialized xorriso version stdout byte count differs.'
        Assert-Equal -Expected $isoCommand.version_stdout_sha256 -Actual ([Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($versionBytes)).ToLowerInvariant()) -Message 'Serialized xorriso version stdout hash differs.'
        $versionText = [System.Text.UTF8Encoding]::new($false, $true).GetString($versionBytes)
        Assert-Equal -Expected $isoCommand.version -Actual $versionText.Substring(0, $versionText.IndexOf("`n", [System.StringComparison]::Ordinal)) -Message 'Serialized xorriso version stdout first line differs from canonical identity.'
